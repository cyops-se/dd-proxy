...
func (listener *UDPMetaListener) run() {
	...
	if err != nil {
		db.Error("UDPMetaListener error", "Failed to listen at address: %s, error: %s", addr.String(), err.Error())
		return
	}
	...
	for {
		...
		if err != nil {
			db.Error("UDPMetaListener error", "Failed to read UDP data (ReadFromUDP), error: %s", err.Error())
			continue
		}

		// Send it over NATS
		fmt.Println(n)
		listener.nc.Publish("meta", p[:n])
		engine.NotifySubscribers("meta", string(p[:n]))
	}
}
